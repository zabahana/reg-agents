...
> Generated by `scripts/generate_model_development_doc.py` on 2026-07-23 01:34 UTC.
...
| scoring holdout (5% of full) | 200 | 193 | 7 | 96.50% |
...
deploys at **0.883**. The test fold plays no role in the choice,
...
|---|---|---|---|
| logistic_regression | TF-IDF 30k, 1-2 gram, sublinear, min_df=2 | L1, C=2.0 (validation-tuned over {l1,l2}×C to close a ~0.20 train/test ROC gap), liblinear, class_weight='balanced' | 0.05 |
| xgboost | same TF-IDF features | 300 trees, depth 6, lr 0.1, hist, aucpr, scale_pos_weight=0.0 | 4.95 |
| lightgbm | same TF-IDF features | 400 trees, 63 leaves, lr 0.05, class_weight='balanced' | 7.03 |
| bert_finetuned | raw text → WordPiece subwords | distilbert-base-uncased, max_len=256, batch 16, AdamW lr 3e-5, 2 epochs, class-weighted cross-entropy | 101.7 |
...
|---|---|---|---|---|---|---|---|---|
| logistic_regression | 0.2396 | 0.639 | 0.211 | 0.8491 | 0.2439 | 0.661 | 0.0475 | 0.05 |
| xgboost | 0.1579 | 0.844 | 0.1938 | 0.7355 | 0.1702 | 0.613 | 0.0395 | 4.95 |
| lightgbm | 0.3011 | 0.883 | 0.2467 | 0.8459 | 0.1429 | 0.5385 | 0.0321 | 7.03 |
| bert_finetuned | 0.1564 | 0.303 | 0.2587 | 0.7724 | 0.3333 | 0.6443 | 0.0354 | 101.7 |
...
|---|---|
| TF-IDF vocabulary (30k, train-fitted) | 2.0% of test tokens out-of-vocabulary |
...
validation-optimized cut-off **0.883**, not the default 0.5; the
...
|---|---|---|---|---|---|
| balanced (deployed) | 0.883 | 0.3011 | 0.2467 | 0.1429 | 0.5385 |
| unweighted | 0.982 | 0.2618 | 0.239 | 0.1176 | 0.5344 |

**Table 13 / Figure 8 — Split-seed stability** (`lightgbm`, five random
80/10/10 re-splits). With only ~13 minority cases per held-out fold,
...
| mean ± std | 0.3259 ± 0.0722 | 0.4029 ± 0.0542 |
...
2. **Minority support.** 135 minority cases overall (~13 per
...
|---|---|---|
| tfidf_vectorizer.joblib | 327 KB | TF-IDF featurizer (30k, 1-2 gram, sublinear, min_df=2), train-fitted |
| model_logistic_regression.joblib | 4 KB | fitted candidate (challenger) |
