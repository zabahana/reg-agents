...
| Generative & Agentic AI | 5 cooperating agents (orchestrator + 4 specialists) |
...
```
reg_agents/
  common/        llm, embeddings, vector store, A2A protocol, MCP client, corpus
  mcp_servers/   regulations / model-registry / fraud  (MCP tool servers, SSE)
  agents/        orchestrator + retriever / validation / fraud / report (A2A)
  ui/            Streamlit demo
data/            regulations corpus, model cards, sample transactions
k8s/             GKE manifests (GPU NIM tier + CPU agents) + setup guide
scripts/         run_local.sh / stop_local.sh / demo_run.py
