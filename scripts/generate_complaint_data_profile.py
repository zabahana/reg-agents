...
def main() -> None:
    ...
    pipeline_rows = [
        ...
        ["4 · Near dedup", "First-200-chars fingerprint + TF-IDF-cosine fuzzy pass "
                           "(≥ 0.9) over the whole pool (Curator: MinHash / FuzzyDuplicates)",
         "removes template complaints that would leak across splits"],
    ...
    update_note = (
        ...
        "stage-1 champion (validation-tuned L1 logistic regression) is "
        "recorded here as part of data governance. **v1** used a single "
        "generic pull of the CFPB narrative stream, which left the "
        ...
        "500-row non-regulatory floor at assembly; the improvement came from "
        "the DATA update alone — same model family, same regularization grid, "
        "same split protocol — lifting test ROC-AUC 0.849 → 0.945 and closing "
        "the generalization gap 0.147 → 0.031. **v3** (current) added a "
        "TF-IDF-cosine fuzzy-dedup pass after a leakage audit found 4.5% of "
        "test rows were near-copies (cosine ≥ 0.9) of training rows — "
        "template complaints whose openings differ, invisible to the 200-char "
        "prefix signature. Removing them makes the headline slightly LOWER "
        "but honest (ROC-AUC 0.936, gap 0.06): the v2 number was partly "
        "inflated by memorized near-duplicates. Leakage checks now run in the "
        "MDD (§9) and the test suite at every retrain."
    )
    update_rows = [
        ["v1 · generic pull only", "135 (3.4%)", "0.639", "0.849", "0.147"],
        ["v2 · + targeted service-issue pass, 500-row floor",
         "500 (12.5%)", "0.397", "0.945", "0.031"],
        ["v3 · + cosine fuzzy dedup, leakage-audited (current)",
         "500 (12.5%)", "0.491", "0.936", "0.060"],
